  ...
  categories: [
      {
          ...
          questions: [
              ...
              {
                  question:
                      "Which unit is responsible for all logical operations in a computer?",
...
function checkIPParticipation() {
  ...
  if (storedIP && storedIP === userIP) {
      alert(
          "You have already participated in this quiz from this IP address."
      );
...
let getResult = JSON.parse(localStorage.getItem("quizResult"));
  // Ensure the user has not already participated
  if (getResult && !checkIPParticipation()) {
...
function captureScreenshotAndOpenGmail() {
  ...
  html2canvas(resultSection).then(function (canvas) {
      ...
      newWindow.document.title =
          "Quiz Result Screenshot - Right-click to Save";
